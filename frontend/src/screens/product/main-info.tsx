...
function ProductMainInfo({ data }: Props) {
  ...
  const handleVariationClick = (variationId: number) => {
    router.push(`/products/${variationId}`);
  };
    ...
    <div className="grid gap-3 w-full">
              ...
              <div className="flex gap-3.5 mt-1">
                <button
                  className={`font-normal p-3 border transition-all duration-200 rounded-[8px] relative ${"border-[#2CA89A] cursor-default border-2"} ${
                    !data.variations.current.in_stock ? "bg-[#F6F6F6] text-[#808080]" : ""
                  }`}
                >
                  {data.variations.current.flavor}
                  {data.variations.current.in_stock || (
                    <span
                      style={{
                        ...
                        width: "100%",
                        height: "100%",
                        background: "linear-gradient(to bottom right,#0000 48%, #E6E6E6 ,#0000 52%)",
                      }}
                    ></span>
                  )}
                </button>
                    ...
                    {variation.in_stock || (
                      <span
                        style={{
                          ...
                          width: "100%",
                          height: "100%",
                          background: "linear-gradient(to bottom right,#0000 47%, #E6E6E6 ,#0000 53%)",
                        }}
                      ></span>
                    )}
                  </button>
          ...
          {data.variations.current.quantity && (
            <div>
              ...
              <div className="flex gap-3.5 mt-1 overflow-auto">
                ...
                >
                  {data.variations.current.quantity}
                  <span className="block font-medium mt-1">{data.variations.current.price}с</span>
                  {data.variations.current.in_stock || (
                    <span
                      style={{
                        position: "absolute",
                ...
                {data.variations?.available_variations
                  ...
                  .map((variation) => (
                    ...
                    >
                      {variation.quantity}
                      <span className="block font-medium mt-1">{variation.price}с</span>
                      {variation.in_stock || (
                        <span
                          style={{
                            position: "absolute",
      ...
      <div className="flex justify-between items-center gap-4">
        <div className="flex gap-2.5">
          <p className="text-xl font-semibold">{data.sale_price || data.price}с</p>
          {data.sale_price && <p className="text-[20px] leading-6 line-through">{data.price}с</p>}
        </div>
        <MainInfoActions />
      </div>
    </div>
...
function Tag({ isHit, isSale }: TagProps) {
  return (
    (isHit || isSale) && (
      <div className={classNames("py-1 px-5 rounded", isHit ? "bg-secondary" : "bg-[#E51616]")}>
        <span className="font-semibold text-white">{isHit ? "Хит" : "Акция"}</span>
      </div>
    )
  );
}
